...
let UserSubscriber = class UserSubscriber {
    ...
    onUserSignUp({ username, email, _id }) {
        const Logger = typedi_1.Container.get('logger');
        try {
            /**
             * @TODO implement this
    ...
    onResendVerification({ username, email, _id }) {
        const Logger = typedi_1.Container.get('logger');
        try {
            /**
             * @TODO implement this
            ...
            Logger.error(`🔥 Error on event ${events_1.default.user.resendVerification}: %o`, e);
            // Throw the error so the process dies (check src/app.ts)
            throw e;
        }
    }
...
__decorate([
    (0, event_dispatch_1.On)(events_1.default.user.resendVerification),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], UserSubscriber.prototype, "onResendVerification", null);
