...
// Utility functions
function getDateRange(numberOfMonths: number, date?: string): { startDate: Date; endDate: Date } {
...
function mapTradeStatus(oldStatus: string): string {
...
// Data operations
async function getAllTrades(): Promise<DatabaseRecord[]> {
    let connection: Connection | undefined;
    try {
        connection = await connectToMySQLDatabase();
        const [rows] = await connection.execute<DatabaseRecord[]>('SELECT * FROM investments');
        return rows;
    } finally {
...
async function bulkCreateTrades(trades: any[]): Promise<any[]> {
    try {
        const result = await Trade.insertMany(trades, {
            ordered: false,
            rawResult: true
...
async function bulkCreateTransactions(transactions: any[]): Promise<any[]> {
    try {
...
async function main() {
    try {
        ...
        const users = await User.find({ "metadata.users_id": {$in: validUserIds} }).exec();
        console.log(`Fetched ${users.length} users from MongoDB`);
        
        // 4. Create user lookup map
        const userMap = new Map(users.map(user => [`${user?.metadata?.users_id}`, user]));
        ...
        const newTrades = oldTrades.map(o_trade => {
            ...
            const dateRange = getDateRange(
                parseInt(o_trade.counter), 
                standardizeDate(o_trade.activeDate)
            );
